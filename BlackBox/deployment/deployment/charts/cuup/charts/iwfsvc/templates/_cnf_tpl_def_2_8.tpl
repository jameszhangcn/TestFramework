...
{{- define "nf.mtcilId" -}}
{{- default "-" $.Values.nf.mtcilId | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{- define "cnfTplHeader_2_8" -}}
{{- $defaultVariables := $.dot.Values.default -}}
{{- $nfVariables := (dict "nfName" (include "nf.nfType" (dict "Values" $.dot.Values) |toString) "nfType" (include "nf.nfClass" (dict "Values" $.dot.Values) |toString) "mtcilId" (include "nf.mtcilId" (dict "Values" $.dot.Values) |toString) "vendorId" (include "nf.vendorId" (dict "Values" $.dot.Values) |toString) "vendorId" (include "nf.vendorId" (dict "Values" $.dot.Values) |toString) "nfId" (include "nf.nfId" (dict "Values" $.dot.Values) |toString) "nfServiceId" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "nfServiceType" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "svcVersion" (include "nf.version" (dict "Values" $.dot.Values) |toString) "nfPrefix" (include "nf.prefix_2_8" (dict "Values" $.dot.Values) |toString) "svcname" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "component_name" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "defaultVariables" ($defaultVariables) "root" ($.dot) "create_meta_ns" "true") -}}
...
{{- define "cnfTplMetadata_2_8" -}}
{{- include "metaSpec_2_8" (merge (dict "specOffset" (add $.setOffset 0 | int) "metaSpec" $.metadata) (dict "nfVariables" $.cnfHdr.nfVariables)) -}}
{{- end -}}
...
{{- include "tmaasSpec_2_8" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
...
{{- ($.nfVariables.root.Files.Glob ($mgmtContents|toString)).AsConfig | trimSuffix "\n" | nindent (add $specOffset 2 | int) }}
{{- end -}}
...
{{- include "configMapDataSpec_2_8" (dict "specOffset" (add $specOffset 0 | int) "configMapDataSpec_2_8" ("") "nfVariables" .nfVariables) -}}
